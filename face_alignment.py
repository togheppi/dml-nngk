...
def pre_process_tf(x):
    # x = (x + 1) / 2
    # x *= 255.0
    x = tf.cast(tf.clip_by_value(x, 0.0, 255.0), dtype=tf.dtypes.uint8)

    return x



def image_align_tf(img, face_landmarks, output_size=1024, transform_size=4096, enable_padding=True):
        # Align function from FFHQ dataset pre-processing step
        # https://github.com/NVlabs/ffhq-dataset/blob/master/download_ffhq.py

        img = pre_process_tf(img)
        ...
        if shrink > 1:
            ...
            img = tf.image.resize(img, rsize, method='lanczos3')
            quad /= shrink
            qsize /= shrink
        ...
        if crop[2] - crop[0] < img.shape[0] or crop[3] - crop[1] < img.shape[1]:
            img = tf.image.crop_to_bounding_box(img, crop[1], crop[0], crop[3] - crop[1], crop[2] - crop[0])
            quad -= crop[0:2]
